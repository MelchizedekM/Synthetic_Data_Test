...
def main():
    ...
    # test
        ...
        if test_acc >best_acc:
            best_acc = test_acc
            # 保存G和D的模型权重
            torch.save(G.state_dict(), os.path.join(opt.save_model, 'G_epoch_{}.pth'.format(epoch)))
            torch.save(D.state_dict(), os.path.join(opt.save_model, 'D_epoch_{}.pth'.format(epoch)))
            #save gen img
        if epoch<=opt.G_epochs: 
